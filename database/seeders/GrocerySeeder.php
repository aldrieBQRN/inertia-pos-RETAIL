...
class GrocerySeeder extends Seeder
{
    public function run()
    {
        ...
        // Ensure we have a store to attach to
        $store = Store::firstOrCreate(
            ['name' => 'SUPER GROCERY'],
            ['address' => 'Downtown', 'status' => true]
        );
        $storeId = $store->id;

        // 2. CREATE CATEGORIES
        $categories = [
            ['store_id' => $storeId, 'name' => 'Fresh Produce', 'created_at' => now(), 'updated_at' => now()],
            ['store_id' => $storeId, 'name' => 'Meat & Seafood', 'created_at' => now(), 'updated_at' => now()],
            ['store_id' => $storeId, 'name' => 'Dairy & Eggs', 'created_at' => now(), 'updated_at' => now()],
            ['store_id' => $storeId, 'name' => 'Beverages', 'created_at' => now(), 'updated_at' => now()],
            ['store_id' => $storeId, 'name' => 'Snacks & Sweets', 'created_at' => now(), 'updated_at' => now()],
            ['store_id' => $storeId, 'name' => 'Household', 'created_at' => now(), 'updated_at' => now()],
        ];
        DB::table('categories')->insert($categories);

        $catIds = DB::table('categories')->where('store_id', $storeId)->pluck('id', 'name');
        ...
        // 4. GENERATE SALES
        $userId = DB::table('users')->where('store_id', $storeId)->first()->id ?? 1;
        ...
        for ($i = 30; $i >= 0; $i--) {
            ...
            for ($j = 0; $j < $dailyOrders; $j++) {
                ...
                $saleId = DB::table('sales')->insertGetId([
                    ...
                    'transaction_date' => $saleDate, // <-- FIX: Added transaction date
                    'created_at' => $saleDate,
                    'updated_at' => $saleDate,
                ]);
