...
<body>
    <div class="container">
        <table class="header-table">
            <tr>
                ...
                <td width="50%" style="text-align: right; vertical-align: bottom;">
                    ...
                    <div style="color: #9ca3af; font-size: 9px;">Generated: {{ now()->format('M d, Y h:i A') }}</div>
                </td>
            </tr>
        </table>
            ...
            <tr>
                ...
                <td width="36%">
                    <div class="stat-card" style="border-color: #dbeafe; background: #eff6ff;">
                        ...
                        <div class="stat-value" style="color: #2563eb;">&#8369;{{ number_format($payments->sum('amount'), 2) }}</div>
                    </div>
                </td>
                ...
                @foreach($payments as $p)
                    <tr>
                        <td>
                            ...
                            <div style="font-size: 8px; color: #6b7280;">Admin: {{ $p->full_name }}</div>
                        </td>
                        <td class="ref-no">{{ $p->reference_number }}</td>
                        <td>
                            <div style="font-weight: bold;">{{ $p->plan->name }}</div>
                        </td>
                        <td>{{ $p->plan->duration_months }} Month(s)</td>
        ...
        <div class="footer">
            ...
            <p>&copy; {{ date('Y') }} {{ $appName }} Internal Control Panel • {{ $supportEmail }}</p>
        </div>
    </div>
</body>
</html>