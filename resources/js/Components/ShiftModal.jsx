    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm sm:p-4 transition-opacity">
            ...
            <div className="relative bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in slide-in-from-bottom-8 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300">
                ...
                <div className="sm:hidden flex justify-center pt-4 pb-2 bg-gray-900 w-full shrink-0" onClick={onClose}>
                ...
                <div className="px-6 py-5 border-b text-white flex justify-between items-center bg-gray-900 shrink-0">
                    ...
                    <button onClick={onClose} className="text-white/50 hover:text-white transition-colors bg-white/5 hover:bg-white/10 p-2 rounded-full hidden sm:block">
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar bg-white">
                    {summary ? (
                        <div className="space-y-4 pb-4 sm:pb-0">
                            ...
                            <div className={`p-4 rounded-lg text-center border shadow-sm ${Math.abs(summary.difference) < 0.01 ? 'bg-green-50 text-green-700 border-green-100' : summary.difference > 0 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
                            ...
                            <div className="grid gap-3 pt-2">
                                <button
                                    ...
                                    className="w-full py-4 bg-gray-900 text-white font-bold rounded-lg shadow-lg flex items-center justify-center gap-2 hover:bg-black transition-all active:scale-[0.98]"
                                >
                                ...
                                <button
                                    ...
                                    className={`w-full flex items-center justify-center gap-2 py-4 border-2 font-bold rounded-lg transition-all active:scale-[0.98] ${isLoggingOut ? 'border-gray-100 text-gray-400 bg-gray-50 cursor-not-allowed' : 'border-blue-100 text-blue-700 bg-blue-50 hover:bg-blue-100'}`}
                                >
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col gap-6 pt-2 pb-6 sm:pb-4">
                            <div>
                                ...
                                <div className="relative group">
                                    <span className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-2xl group-focus-within:text-emerald-500 transition-colors">₱</span>
                                    <input
                                        type="number"
                                        step="0.01"
                                        required
                                        autoFocus
                                        className="w-full pl-14 pr-6 py-4 text-3xl font-black border-2 border-gray-100 rounded-lg text-center text-gray-900 focus:border-emerald-500 focus:ring-0 transition-all bg-gray-50 focus:bg-white outline-none"
                                        placeholder="0.00"
                                        value={amount}
                            ...
                            <button type="submit" disabled={loading || !amount} className={`w-full mt-2 py-4 text-white font-black text-lg rounded-lg sm:rounded-lg shadow-lg transition-all active:scale-[0.98] ${loading || !amount ? 'bg-gray-300 cursor-not-allowed shadow-none' : 'bg-gray-900 hover:bg-black'}`}>
