...
export default function BillingPortal({ auth, store, plans, pendingPayment, history, paymentMethods }) {
    ...
    const handleOpenHistoryModal = async () => {
        ...
        try {
            const response = await fetch(route('tenant.billing.history'));
    ...
    const handlePaymentMethodSelect = (method) => {
        ...
        setData('payment_method', method.type);
    };
        ...
        <BillingLayout storeName={store.name}>
                ...
                <div className="flex-1 space-y-8">
                    ...
                    <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Recent History</h3>
                            {history.length > 0 && (
                                <button
            ...
            {showHistoryModal && (
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
                    {/* Backdrop */}
                    <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm" onClick={() => setShowHistoryModal(false)}></div>

                    {/* Modal Content */}
                    <div className="bg-white rounded-3xl w-full max-w-xl max-h-[80vh] flex flex-col shadow-2xl border border-gray-100 overflow-hidden relative z-10 animate-in zoom-in-95 duration-200">
                        {/* Header */}
                        <div className="bg-gray-900 text-white p-6 flex justify-between items-center select-none">
                            <div>
                                <h3 className="text-sm font-black uppercase tracking-widest">Transaction History</h3>
                                <p className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter mt-1">All Recorded Renewals</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 overflow-y-auto space-y-4 flex-1">
                            {isLoadingHistory ? (
                                ...
                                <p className="text-xs font-bold text-gray-300 uppercase italic py-16 text-center">No transactions found.</p>
                            ) : (
                                <div className="space-y-3">
                                    {fullHistory.map(item => (
                                        <div key={item.id} className="p-5 bg-gray-50/50 border border-gray-150 rounded-2xl flex flex-col gap-3.5 hover:bg-gray-50 transition-colors">
                                            ...
                                            {item.status === 'rejected' && item.rejection_reason && (
                                                <div className="p-3 rounded-xl bg-red-50 border border-red-100 text-red-700 text-[10px] font-bold flex gap-2">
                                                    <svg className="w-3.5 h-3.5 text-red-500 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                                                    <div className="flex-1 leading-normal">
                                                        <span className="block font-black uppercase tracking-wider text-[8px] text-red-600 mb-0.5">Rejection Reason</span>
                                                        {item.rejection_reason}
